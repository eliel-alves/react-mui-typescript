...
const getAll = async (page = 1, filter = '', id = ''): Promise<TCityTotalCount | Error> => {
  try {
    const relativeUrl = `/cities?_page=${page}&_limit=${Environment.LINES_LIMIT}&name_like=${filter}&id_like=${id}`;
